...
listegenre = []
listeLivres = []
#Nous listons tous les genres de livres
for i in nav:
...
liste_category_url = [] #toutes les urls de chaque categories avec leur pages associées LEN = 80
for numeroPage, genre in enumerate(listegenre):
    current = "current"
    linkurl = requests.get(f"http://books.toscrape.com/catalogue/category/books/{genre}_{numeroPage + 2}/index.html")
    ...
    if current in str(linkurl.content):
        ...
        while i <= int(pagecount):
            linkurl = requests.get(f"http://books.toscrape.com/catalogue/category/books/{genre}_{numeroPage + 2}/page-{i}.html")
            x = f"http://books.toscrape.com/catalogue/category/books/{genre}_{numeroPage + 2}/page-{i}.html"
            i = i + 1
            liste_category_url.append(x)
    else: liste_category_url.append(x)
...
for i in range(len(listegenre)):
    header = ["title", "category", "review_rating", "universal_ product_code (upc)", "price_excluding_tax", "price_including_tax", "number_available", "image_url", "product_page_url", "product_description"]
...
liste_livre_url = [] #Liste de toutes les urls par categorie LEN = 1000
for i in range(80):
...
for a in range(1000):
    ...
    informations = {"title": liste_titles[a], "category": liste_category[a], "review_rating": liste_review[a], "universal_ product_code (upc)": upcs[a],
